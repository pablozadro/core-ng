...
export class DashboardFilterComponent implements OnInit {
  ...
  orderControlOptions: CoreSelectOption[] = [
    ...
    { label: 'protein - ASC', value: 'proteins 1' },
    { label: 'protein - DESC', value: 'proteins -1' },
    { label: 'calories - ASC', value: 'calories 1' },
    { label: 'calories - DESC', value: 'calories -1' },
  ];
  ...
  ngOnInit(): void {
    ...
    this.query$.subscribe(query => {
      ...
      if(!this.queryState.orderBy && this.queryState.orderDir) {
        const current = this.orderControl.value;
        if(!current) {
          this.orderControl.setValue(`protein ${this.queryState.orderDir}`)
        } else {
